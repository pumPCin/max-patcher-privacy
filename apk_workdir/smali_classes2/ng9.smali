.class public final synthetic Lng9;
.super Lti6;
.source "SourceFile"

# interfaces
.implements Lqh6;


# instance fields
.field public final synthetic a:I


# direct methods
.method public synthetic constructor <init>(ILjava/lang/Object;Ljava/lang/Class;Ljava/lang/String;Ljava/lang/String;II)V
    .locals 0

    .line 1
    iput p7, p0, Lng9;->a:I

    move-object p7, p4

    move-object p4, p3

    move p3, p6

    move-object p6, p7

    move-object p7, p5

    move-object p5, p2

    move p2, p1

    move-object p1, p0

    invoke-direct/range {p1 .. p7}, Lsi6;-><init>(IILjava/lang/Class;Ljava/lang/Object;Ljava/lang/String;Ljava/lang/String;)V

    return-void
.end method

.method public constructor <init>(Loo1;I)V
    .locals 7

    iput p2, p0, Lng9;->a:I

    packed-switch p2, :pswitch_data_0

    .line 2
    const-string v6, "onAllParticipantsLoadError(Ljava/lang/Throwable;)V"

    const/4 v2, 0x0

    const/4 v1, 0x1

    .line 3
    const-class v3, Loo1;

    const-string v5, "onAllParticipantsLoadError"

    move-object v0, p0

    move-object v4, p1

    invoke-direct/range {v0 .. v6}, Lsi6;-><init>(IILjava/lang/Class;Ljava/lang/Object;Ljava/lang/String;Ljava/lang/String;)V

    return-void

    .line 4
    :pswitch_0
    const-string v6, "onAllRoomsLoadError(Ljava/lang/Throwable;)V"

    const/4 v2, 0x0

    const/4 v1, 0x1

    .line 5
    const-class v3, Loo1;

    const-string v5, "onAllRoomsLoadError"

    move-object v0, p0

    move-object v4, p1

    invoke-direct/range {v0 .. v6}, Lsi6;-><init>(IILjava/lang/Class;Ljava/lang/Object;Ljava/lang/String;Ljava/lang/String;)V

    return-void

    .line 6
    :pswitch_1
    const-string v6, "onAllRoomsLoaded(Lru/ok/android/webrtc/signaling/sessionroom/event/SignalingSessionRooms;)V"

    const/4 v2, 0x0

    const/4 v1, 0x1

    .line 7
    const-class v3, Loo1;

    const-string v5, "onAllRoomsLoaded"

    move-object v0, p0

    move-object v4, p1

    invoke-direct/range {v0 .. v6}, Lsi6;-><init>(IILjava/lang/Class;Ljava/lang/Object;Ljava/lang/String;Ljava/lang/String;)V

    return-void

    nop

    :pswitch_data_0
    .packed-switch 0x19
        :pswitch_1
        :pswitch_0
    .end packed-switch
.end method


# virtual methods
.method public final invoke(Ljava/lang/Object;)Ljava/lang/Object;
    .locals 21

    move-object/from16 v0, p0

    iget v1, v0, Lng9;->a:I

    const/4 v2, 0x3

    const-string v3, "CallSessionRoomsManager"

    const/4 v4, 0x2

    const/4 v5, 0x0

    const/4 v6, 0x0

    const/4 v7, 0x1

    packed-switch v1, :pswitch_data_0

    move-object/from16 v1, p1

    check-cast v1, Ljava/lang/Throwable;

    iget-object v2, v0, Lst1;->receiver:Ljava/lang/Object;

    check-cast v2, Loo1;

    iget-object v2, v2, Loo1;->a:Lyuc;

    const-string v4, "All rooms load error"

    invoke-interface {v2, v3, v4, v1}, Lyuc;->logException(Ljava/lang/String;Ljava/lang/String;Ljava/lang/Throwable;)V

    sget-object v1, Lzag;->a:Lzag;

    return-object v1

    :pswitch_0
    move-object/from16 v1, p1

    check-cast v1, Ldne;

    iget-object v2, v0, Lst1;->receiver:Ljava/lang/Object;

    check-cast v2, Loo1;

    invoke-virtual {v2, v1}, Loo1;->e(Ldne;)V

    sget-object v1, Lzag;->a:Lzag;

    return-object v1

    :pswitch_1
    move-object/from16 v1, p1

    check-cast v1, Ljava/lang/Throwable;

    iget-object v2, v0, Lst1;->receiver:Ljava/lang/Object;

    check-cast v2, Loo1;

    iget-object v2, v2, Loo1;->a:Lyuc;

    const-string v4, "All participants load error"

    invoke-interface {v2, v3, v4, v1}, Lyuc;->logException(Ljava/lang/String;Ljava/lang/String;Ljava/lang/Throwable;)V

    sget-object v1, Lzag;->a:Lzag;

    return-object v1

    :pswitch_2
    move-object/from16 v1, p1

    check-cast v1, Ljava/lang/String;

    iget-object v2, v0, Lst1;->receiver:Ljava/lang/Object;

    check-cast v2, Lgvg;

    iget-object v2, v2, Ltq3;->c:Ljava/lang/Object;

    check-cast v2, Lyuc;

    const-string v3, "VideoRecord_BufferTransform"

    invoke-interface {v2, v3, v1}, Lyuc;->log(Ljava/lang/String;Ljava/lang/String;)V

    sget-object v1, Lzag;->a:Lzag;

    return-object v1

    :pswitch_3
    move-object/from16 v1, p1

    check-cast v1, Lzh1;

    iget-object v2, v0, Lst1;->receiver:Ljava/lang/Object;

    check-cast v2, Ldi;

    iget-object v3, v2, Ldi;->a:Lyz0;

    iget-object v4, v3, Lyz0;->m:Lyh1;

    iget-object v4, v4, Lyh1;->z:Lwh1;

    iget-boolean v4, v4, Lwh1;->m:Z

    if-eqz v4, :cond_0

    iget-object v3, v3, Lyz0;->n0:Lhr1;

    invoke-virtual {v3}, Lhr1;->v()Lmyf;

    move-result-object v3

    sget-object v4, Lmyf;->c:Lmyf;

    if-ne v3, v4, :cond_0

    iget-object v2, v2, Ldi;->a:Lyz0;

    iget-object v2, v2, Lyz0;->j0:Lii1;

    iget-object v2, v2, Lii1;->a:Ldi1;

    iget-object v2, v2, Ldi1;->a:Lzh1;

    invoke-static {v1, v2}, Lc0i;->a(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v7

    :cond_0
    invoke-static {v7}, Ljava/lang/Boolean;->valueOf(Z)Ljava/lang/Boolean;

    move-result-object v1

    return-object v1

    :pswitch_4
    move-object/from16 v1, p1

    check-cast v1, Lt7;

    iget-object v2, v0, Lst1;->receiver:Ljava/lang/Object;

    check-cast v2, Lu7;

    invoke-virtual {v2}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    iget-object v3, v1, Lt7;->b:Ljava/lang/String;

    invoke-static {v3}, Lru/ok/android/externcalls/analytics/events/EventItemValueKt;->toEventItemValue(Ljava/lang/String;)Lru/ok/android/externcalls/analytics/events/EventItemValue;

    move-result-object v3

    iget-object v1, v1, Lt7;->a:Lk68;

    iget-object v1, v1, Lk68;->o:Ljava/lang/Object;

    check-cast v1, Ljava/lang/String;

    if-nez v1, :cond_1

    const-string v1, "NULL"

    :cond_1
    invoke-static {v1}, Lru/ok/android/externcalls/analytics/events/EventItemValueKt;->toEventItemValue(Ljava/lang/String;)Lru/ok/android/externcalls/analytics/events/EventItemValue;

    move-result-object v1

    iget-object v2, v2, Lu7;->a:Lq71;

    const-wide/16 v4, 0x0

    invoke-static {v4, v5}, Lru/ok/android/externcalls/analytics/events/EventItemValueKt;->toEventItemValue(J)Lru/ok/android/externcalls/analytics/events/EventItemValue;

    move-result-object v4

    new-instance v5, Lru/ok/android/externcalls/analytics/events/EventItemsMap;

    new-instance v6, Lqbb;

    const-string v7, "codec_implementation"

    invoke-direct {v6, v7, v3}, Lqbb;-><init>(Ljava/lang/Object;Ljava/lang/Object;)V

    new-instance v3, Lqbb;

    const-string v7, "string_value"

    invoke-direct {v3, v7, v1}, Lqbb;-><init>(Ljava/lang/Object;Ljava/lang/Object;)V

    filled-new-array {v6, v3}, [Lqbb;

    move-result-object v1

    invoke-static {v1}, Lyf8;->f([Lqbb;)Ljava/util/Map;

    move-result-object v1

    invoke-direct {v5, v1}, Lru/ok/android/externcalls/analytics/events/EventItemsMap;-><init>(Ljava/util/Map;)V

    const-string v1, "codec_usage"

    invoke-virtual {v2, v1, v4, v5}, Lq71;->c(Ljava/lang/String;Lru/ok/android/externcalls/analytics/events/EventItemValue;Lru/ok/android/externcalls/analytics/events/EventItemsMap;)V

    sget-object v1, Lzag;->a:Lzag;

    return-object v1

    :pswitch_5
    move-object/from16 v1, p1

    check-cast v1, Lmih;

    iget-object v2, v0, Lst1;->receiver:Ljava/lang/Object;

    check-cast v2, Lnih;

    invoke-interface {v2, v1}, Lnih;->a(Lmih;)V

    sget-object v1, Lzag;->a:Lzag;

    return-object v1

    :pswitch_6
    move-object/from16 v1, p1

    check-cast v1, Ldsf;

    iget-object v2, v0, Lst1;->receiver:Ljava/lang/Object;

    check-cast v2, Lyr;

    iget-object v2, v2, Lyr;->a:Lgs;

    iget-object v3, v2, Lgs;->u0:Llt7;

    invoke-interface {v3}, Llt7;->getValue()Ljava/lang/Object;

    move-result-object v3

    check-cast v3, Lqkf;

    check-cast v3, Losa;

    invoke-virtual {v3}, Losa;->b()Lv44;

    move-result-object v3

    new-instance v7, Lfs;

    invoke-direct {v7, v2, v1, v6}, Lfs;-><init>(Lgs;Ldsf;Lkotlin/coroutines/Continuation;)V

    invoke-static {v2, v3, v7, v4}, Lsyg;->m(Lsyg;Lt44;Lei6;I)Lwwe;

    move-result-object v1

    iget-object v3, v2, Lgs;->H0:Lpzd;

    sget-object v4, Lgs;->K0:[Lwq7;

    aget-object v4, v4, v5

    invoke-virtual {v3, v2, v4, v1}, Lpzd;->P(Ljava/lang/Object;Lwq7;Ljava/lang/Object;)V

    sget-object v1, Lzag;->a:Lzag;

    return-object v1

    :pswitch_7
    move-object/from16 v1, p1

    check-cast v1, Loe9;

    iget-object v2, v0, Lst1;->receiver:Ljava/lang/Object;

    check-cast v2, Lva9;

    invoke-virtual {v2}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    invoke-static {v1}, Lva9;->a(Loe9;)Lwa9;

    move-result-object v1

    return-object v1

    :pswitch_8
    move-object/from16 v1, p1

    check-cast v1, Landroid/view/MotionEvent;

    iget-object v3, v0, Lst1;->receiver:Ljava/lang/Object;

    check-cast v3, Lygf;

    iget v6, v3, Lygf;->l:I

    iget v8, v3, Lygf;->l:I

    iget v9, v3, Lygf;->k:I

    iget v10, v3, Lygf;->e:I

    iget-object v11, v3, Lygf;->o:Llbd;

    iget-object v12, v3, Lygf;->b:Landroid/view/View;

    iget-object v13, v3, Lygf;->a:Lnfd;

    invoke-virtual {v13}, Lnfd;->invoke()Ljava/lang/Object;

    move-result-object v13

    check-cast v13, Ljava/lang/Boolean;

    invoke-virtual {v13}, Ljava/lang/Boolean;->booleanValue()Z

    move-result v13

    if-nez v13, :cond_2

    goto/16 :goto_16

    :cond_2
    invoke-virtual {v1}, Landroid/view/MotionEvent;->getPointerCount()I

    move-result v13

    if-le v13, v7, :cond_4

    iget-boolean v13, v3, Lygf;->f:Z

    if-eqz v13, :cond_4

    invoke-virtual {v3}, Lygf;->b()Z

    move-result v1

    if-eqz v1, :cond_3

    invoke-virtual {v12}, Landroid/view/View;->getTranslationX()F

    move-result v1

    int-to-float v2, v9

    :goto_0
    div-float/2addr v1, v2

    goto :goto_1

    :cond_3
    invoke-virtual {v12}, Landroid/view/View;->getTranslationY()F

    move-result v1

    int-to-float v2, v8

    goto :goto_0

    :goto_1
    invoke-virtual {v3, v1, v7}, Lygf;->c(FZ)V

    goto/16 :goto_16

    :cond_4
    invoke-virtual {v11}, Llbd;->getValue()Ljava/lang/Object;

    move-result-object v13

    check-cast v13, Landroid/view/VelocityTracker;

    invoke-virtual {v13, v1}, Landroid/view/VelocityTracker;->addMovement(Landroid/view/MotionEvent;)V

    invoke-virtual {v1}, Landroid/view/MotionEvent;->getAction()I

    move-result v13

    const/4 v14, 0x0

    if-eq v13, v7, :cond_13

    invoke-virtual {v1}, Landroid/view/MotionEvent;->getAction()I

    move-result v13

    if-ne v13, v2, :cond_5

    goto/16 :goto_d

    :cond_5
    iget-boolean v2, v3, Lygf;->f:Z

    if-eqz v2, :cond_c

    if-nez v2, :cond_6

    goto/16 :goto_15

    :cond_6
    invoke-virtual {v3}, Lygf;->b()Z

    move-result v2

    if-eqz v2, :cond_7

    iget v2, v3, Lygf;->g:F

    goto :goto_2

    :cond_7
    iget v2, v3, Lygf;->h:F

    :goto_2
    cmpl-float v4, v2, v14

    if-lez v4, :cond_21

    invoke-virtual {v3}, Lygf;->b()Z

    move-result v4

    if-eqz v4, :cond_8

    invoke-virtual {v1}, Landroid/view/MotionEvent;->getRawX()F

    move-result v4

    :goto_3
    sub-float/2addr v2, v4

    goto :goto_4

    :cond_8
    invoke-virtual {v1}, Landroid/view/MotionEvent;->getRawY()F

    move-result v4

    goto :goto_3

    :goto_4
    invoke-virtual {v3}, Lygf;->b()Z

    move-result v4

    if-eqz v4, :cond_a

    invoke-virtual {v12}, Landroid/view/View;->getTranslationX()F

    move-result v4

    sub-float/2addr v4, v2

    cmpg-float v2, v4, v14

    if-gez v2, :cond_9

    goto :goto_5

    :cond_9
    move v14, v4

    :goto_5
    int-to-float v2, v9

    div-float/2addr v14, v2

    goto :goto_6

    :cond_a
    invoke-virtual {v12}, Landroid/view/View;->getTranslationY()F

    move-result v4

    sub-float/2addr v4, v2

    int-to-float v2, v6

    div-float v14, v4, v2

    :goto_6
    invoke-virtual {v3, v14}, Lygf;->d(F)V

    iget-object v2, v3, Lygf;->q:Lone/me/sdk/conductor/changehandlers/swipe/SwipeWidget;

    if-eqz v2, :cond_b

    invoke-virtual {v2}, Lone/me/sdk/conductor/changehandlers/swipe/SwipeWidget;->G0()V

    :cond_b
    invoke-virtual {v1}, Landroid/view/MotionEvent;->getRawX()F

    move-result v2

    iput v2, v3, Lygf;->g:F

    invoke-virtual {v1}, Landroid/view/MotionEvent;->getRawY()F

    move-result v1

    iput v1, v3, Lygf;->h:F

    goto/16 :goto_15

    :cond_c
    iget v2, v3, Lygf;->g:F

    cmpl-float v2, v2, v14

    if-lez v2, :cond_11

    iget v2, v3, Lygf;->h:F

    cmpl-float v2, v2, v14

    if-lez v2, :cond_11

    invoke-virtual {v3}, Lygf;->b()Z

    move-result v2

    if-eqz v2, :cond_d

    iget v2, v3, Lygf;->i:F

    invoke-virtual {v1}, Landroid/view/MotionEvent;->getRawX()F

    move-result v5

    :goto_7
    sub-float/2addr v2, v5

    goto :goto_8

    :cond_d
    iget v2, v3, Lygf;->j:F

    invoke-virtual {v1}, Landroid/view/MotionEvent;->getRawY()F

    move-result v5

    goto :goto_7

    :goto_8
    invoke-virtual {v3}, Lygf;->b()Z

    move-result v5

    if-eqz v5, :cond_e

    iget v5, v3, Lygf;->j:F

    invoke-virtual {v1}, Landroid/view/MotionEvent;->getRawY()F

    move-result v6

    :goto_9
    sub-float/2addr v5, v6

    goto :goto_a

    :cond_e
    iget v5, v3, Lygf;->i:F

    invoke-virtual {v1}, Landroid/view/MotionEvent;->getRawX()F

    move-result v6

    goto :goto_9

    :goto_a
    invoke-static {v10}, Lwx1;->v(I)I

    move-result v6

    if-eqz v6, :cond_10

    if-ne v6, v7, :cond_f

    cmpg-float v6, v2, v14

    if-gez v6, :cond_12

    goto :goto_b

    :cond_f
    new-instance v1, Lkotlin/NoWhenBranchMatchedException;

    invoke-direct {v1}, Lkotlin/NoWhenBranchMatchedException;-><init>()V

    throw v1

    :cond_10
    :goto_b
    invoke-static {v2}, Ljava/lang/Math;->abs(F)F

    move-result v6

    iget-object v8, v3, Lygf;->p:Ljava/lang/Object;

    invoke-interface {v8}, Llt7;->getValue()Ljava/lang/Object;

    move-result-object v8

    check-cast v8, Ljava/lang/Number;

    invoke-virtual {v8}, Ljava/lang/Number;->intValue()I

    move-result v8

    int-to-float v8, v8

    cmpl-float v6, v6, v8

    if-lez v6, :cond_12

    invoke-static {v2}, Ljava/lang/Math;->abs(F)F

    move-result v2

    invoke-static {v5}, Ljava/lang/Math;->abs(F)F

    move-result v5

    int-to-float v4, v4

    mul-float/2addr v5, v4

    cmpl-float v2, v2, v5

    if-lez v2, :cond_12

    iput-boolean v7, v3, Lygf;->f:Z

    invoke-virtual {v12}, Landroid/view/View;->getParent()Landroid/view/ViewParent;

    move-result-object v2

    invoke-interface {v2, v7}, Landroid/view/ViewParent;->requestDisallowInterceptTouchEvent(Z)V

    iget-object v2, v3, Lygf;->c:Landroid/view/ViewGroup;

    iget-object v4, v3, Lygf;->n:Ly6;

    invoke-virtual {v2, v12}, Landroid/view/ViewGroup;->indexOfChild(Landroid/view/View;)I

    move-result v5

    invoke-virtual {v2, v4, v5}, Landroid/view/ViewGroup;->addView(Landroid/view/View;I)V

    iget-object v2, v3, Lygf;->q:Lone/me/sdk/conductor/changehandlers/swipe/SwipeWidget;

    if-eqz v2, :cond_12

    iput-boolean v7, v2, Lone/me/sdk/conductor/changehandlers/swipe/SwipeWidget;->b:Z

    invoke-virtual {v2}, Lone/me/sdk/conductor/changehandlers/swipe/SwipeWidget;->H0()V

    goto :goto_c

    :cond_11
    invoke-virtual {v1}, Landroid/view/MotionEvent;->getRawX()F

    move-result v2

    iput v2, v3, Lygf;->i:F

    invoke-virtual {v1}, Landroid/view/MotionEvent;->getRawY()F

    move-result v2

    iput v2, v3, Lygf;->j:F

    :cond_12
    :goto_c
    invoke-virtual {v1}, Landroid/view/MotionEvent;->getRawX()F

    move-result v2

    iput v2, v3, Lygf;->g:F

    invoke-virtual {v1}, Landroid/view/MotionEvent;->getRawY()F

    move-result v1

    iput v1, v3, Lygf;->h:F

    goto/16 :goto_15

    :cond_13
    :goto_d
    iget-boolean v2, v3, Lygf;->f:Z

    const/high16 v13, -0x40800000    # -1.0f

    if-nez v2, :cond_14

    iput-boolean v5, v3, Lygf;->f:Z

    iput v13, v3, Lygf;->g:F

    iput v13, v3, Lygf;->h:F

    goto/16 :goto_15

    :cond_14
    invoke-virtual {v11}, Llbd;->getValue()Ljava/lang/Object;

    move-result-object v2

    check-cast v2, Landroid/view/VelocityTracker;

    invoke-virtual {v2, v7}, Landroid/view/VelocityTracker;->computeCurrentVelocity(I)V

    invoke-virtual {v3}, Lygf;->b()Z

    move-result v2

    if-eqz v2, :cond_15

    invoke-virtual {v11}, Llbd;->getValue()Ljava/lang/Object;

    move-result-object v2

    check-cast v2, Landroid/view/VelocityTracker;

    invoke-virtual {v1}, Landroid/view/MotionEvent;->getActionIndex()I

    move-result v15

    invoke-virtual {v1, v15}, Landroid/view/MotionEvent;->getPointerId(I)I

    move-result v1

    invoke-virtual {v2, v1}, Landroid/view/VelocityTracker;->getXVelocity(I)F

    move-result v1

    goto :goto_e

    :cond_15
    invoke-virtual {v11}, Llbd;->getValue()Ljava/lang/Object;

    move-result-object v2

    check-cast v2, Landroid/view/VelocityTracker;

    invoke-virtual {v1}, Landroid/view/MotionEvent;->getActionIndex()I

    move-result v15

    invoke-virtual {v1, v15}, Landroid/view/MotionEvent;->getPointerId(I)I

    move-result v1

    invoke-virtual {v2, v1}, Landroid/view/VelocityTracker;->getYVelocity(I)F

    move-result v1

    :goto_e
    invoke-virtual {v11}, Llbd;->getValue()Ljava/lang/Object;

    move-result-object v2

    check-cast v2, Landroid/view/VelocityTracker;

    invoke-virtual {v2}, Landroid/view/VelocityTracker;->recycle()V

    sget-object v2, Lzw3;->t0:Lzw3;

    iput-object v2, v11, Llbd;->b:Ljava/lang/Object;

    invoke-static {v10}, Lwx1;->v(I)I

    move-result v2

    if-eqz v2, :cond_16

    if-ne v2, v7, :cond_18

    cmpl-float v2, v1, v14

    if-lez v2, :cond_17

    :cond_16
    move v2, v7

    goto :goto_f

    :cond_17
    move v2, v5

    goto :goto_f

    :cond_18
    new-instance v1, Lkotlin/NoWhenBranchMatchedException;

    invoke-direct {v1}, Lkotlin/NoWhenBranchMatchedException;-><init>()V

    throw v1

    :goto_f
    invoke-virtual {v3}, Lygf;->b()Z

    move-result v10

    if-eqz v10, :cond_19

    invoke-virtual {v12}, Landroid/view/View;->getTranslationX()F

    move-result v10

    goto :goto_10

    :cond_19
    invoke-virtual {v12}, Landroid/view/View;->getTranslationY()F

    move-result v10

    :goto_10
    invoke-virtual {v3}, Lygf;->b()Z

    move-result v11

    if-eqz v11, :cond_1a

    invoke-virtual {v12}, Landroid/view/View;->getTranslationX()F

    move-result v8

    int-to-float v11, v9

    div-float/2addr v8, v11

    goto :goto_11

    :cond_1a
    invoke-virtual {v12}, Landroid/view/View;->getTranslationY()F

    move-result v11

    int-to-float v8, v8

    div-float v8, v11, v8

    :goto_11
    if-eqz v2, :cond_1b

    invoke-static {v1}, Ljava/lang/Math;->abs(F)F

    move-result v1

    const/high16 v2, 0x3fc00000    # 1.5f

    cmpl-float v1, v1, v2

    if-gez v1, :cond_1d

    :cond_1b
    invoke-virtual {v3}, Lygf;->b()Z

    move-result v1

    const v2, 0x3e4ccccd    # 0.2f

    if-eqz v1, :cond_1c

    int-to-float v1, v9

    div-float/2addr v10, v1

    invoke-static {v10}, Ljava/lang/Math;->abs(F)F

    move-result v1

    cmpl-float v1, v1, v2

    if-lez v1, :cond_20

    goto :goto_12

    :cond_1c
    int-to-float v1, v6

    div-float/2addr v10, v1

    invoke-static {v10}, Ljava/lang/Math;->abs(F)F

    move-result v1

    cmpl-float v1, v1, v2

    if-lez v1, :cond_20

    :cond_1d
    :goto_12
    iget-object v1, v3, Lygf;->t:Landroid/animation/ValueAnimator;

    if-eqz v1, :cond_1e

    invoke-virtual {v1}, Landroid/animation/ValueAnimator;->isRunning()Z

    move-result v1

    if-ne v1, v7, :cond_1e

    goto :goto_14

    :cond_1e
    cmpg-float v1, v8, v14

    const/high16 v2, 0x3f800000    # 1.0f

    if-gez v1, :cond_1f

    move v1, v13

    goto :goto_13

    :cond_1f
    move v1, v2

    :goto_13
    invoke-static {v8}, Ljava/lang/Math;->abs(F)F

    move-result v6

    sub-float/2addr v2, v6

    const-wide/16 v9, 0xc8

    long-to-float v6, v9

    mul-float/2addr v2, v6

    float-to-long v14, v2

    const-wide/16 v16, 0x0

    const-wide/16 v18, 0xc8

    invoke-static/range {v14 .. v19}, Ljvi;->e(JJJ)J

    move-result-wide v9

    new-array v2, v4, [F

    aput v8, v2, v5

    aput v1, v2, v7

    invoke-static {v2}, Landroid/animation/ValueAnimator;->ofFloat([F)Landroid/animation/ValueAnimator;

    move-result-object v1

    invoke-virtual {v1, v9, v10}, Landroid/animation/ValueAnimator;->setDuration(J)Landroid/animation/ValueAnimator;

    move-result-object v1

    new-instance v2, Lxgf;

    invoke-direct {v2, v3, v5}, Lxgf;-><init>(Lygf;I)V

    invoke-virtual {v1, v2}, Landroid/animation/ValueAnimator;->addUpdateListener(Landroid/animation/ValueAnimator$AnimatorUpdateListener;)V

    new-instance v2, Ltf;

    invoke-direct {v2, v3, v8}, Ltf;-><init>(Lygf;F)V

    invoke-virtual {v1, v2}, Landroid/animation/Animator;->addListener(Landroid/animation/Animator$AnimatorListener;)V

    iput-object v1, v3, Lygf;->t:Landroid/animation/ValueAnimator;

    invoke-virtual {v1}, Landroid/animation/ValueAnimator;->start()V

    goto :goto_14

    :cond_20
    invoke-virtual {v3, v8, v5}, Lygf;->c(FZ)V

    :goto_14
    iput-boolean v5, v3, Lygf;->f:Z

    iput v13, v3, Lygf;->g:F

    iput v13, v3, Lygf;->h:F

    :cond_21
    :goto_15
    iget-boolean v5, v3, Lygf;->f:Z

    :goto_16
    invoke-static {v5}, Ljava/lang/Boolean;->valueOf(Z)Ljava/lang/Boolean;

    move-result-object v1

    return-object v1

    :pswitch_9
    move-object/from16 v1, p1

    check-cast v1, Landroid/view/View;

    iget-object v2, v0, Lst1;->receiver:Ljava/lang/Object;

    check-cast v2, Lone/me/stickerssettings/stickersscreen/StickersScreen;

    sget-object v3, Lone/me/stickerssettings/stickersscreen/StickersScreen;->u0:[Lwq7;

    invoke-virtual {v2}, Lone/me/stickerssettings/stickersscreen/StickersScreen;->E0()Lh7f;

    move-result-object v3

    iget-object v3, v3, Lh7f;->y0:Lgzc;

    iget-object v3, v3, Lgzc;->a:Llze;

    invoke-interface {v3}, Llze;->getValue()Ljava/lang/Object;

    move-result-object v3

    check-cast v3, Lt6f;

    if-eqz v3, :cond_22

    iget-object v6, v3, Lt6f;->d:Ljava/util/List;

    :cond_22
    if-eqz v6, :cond_24

    invoke-interface {v6}, Ljava/util/Collection;->isEmpty()Z

    move-result v3

    if-eqz v3, :cond_23

    goto :goto_17

    :cond_23
    invoke-static {v7}, Lcvi;->a(I)Ls04;

    move-result-object v3

    invoke-interface {v3, v6}, Ls04;->i(Ljava/util/Collection;)Ls04;

    move-result-object v3

    invoke-interface {v3, v1}, Ls04;->t(Landroid/view/View;)Ls04;

    move-result-object v1

    invoke-interface {v1}, Ls04;->c()Ls04;

    move-result-object v1

    invoke-interface {v1}, Ls04;->build()Lt04;

    move-result-object v1

    invoke-interface {v1, v2}, Lt04;->v(Lone/me/sdk/arch/Widget;)V

    :cond_24
    :goto_17
    sget-object v1, Lzag;->a:Lzag;

    return-object v1

    :pswitch_a
    move-object/from16 v1, p1

    check-cast v1, Ltr6;

    iget-object v2, v0, Lst1;->receiver:Ljava/lang/Object;

    check-cast v2, Lone/me/location/map/show/ShowLocationScreen;

    invoke-virtual {v2, v1}, Lone/me/location/map/show/ShowLocationScreen;->W(Ltr6;)V

    sget-object v1, Lzag;->a:Lzag;

    return-object v1

    :pswitch_b
    move-object/from16 v1, p1

    check-cast v1, Leg1;

    iget-object v2, v0, Lst1;->receiver:Ljava/lang/Object;

    check-cast v2, Lgg1;

    iget-object v3, v2, Lgg1;->c:Lxe;

    iget-object v4, v1, Leg1;->k:Ljc8;

    invoke-virtual {v4, v3}, Landroid/os/Handler;->removeCallbacks(Ljava/lang/Runnable;)V

    iget-object v3, v2, Lgg1;->a:Lyuc;

    iget-object v4, v2, Lgg1;->d:Ljava/lang/String;

    const-string v7, "Statistics report task cancelled"

    invoke-interface {v3, v4, v7}, Lyuc;->log(Ljava/lang/String;Ljava/lang/String;)V

    iget-object v7, v2, Lgg1;->i:Ljava/util/ArrayList;

    invoke-virtual {v7}, Ljava/util/ArrayList;->size()I

    move-result v8

    new-instance v9, Ljava/lang/StringBuilder;

    const-string v10, "Will now release "

    invoke-direct {v9, v10}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    invoke-virtual {v9, v8}, Ljava/lang/StringBuilder;->append(I)Ljava/lang/StringBuilder;

    const-string v8, " registered drawers"

    invoke-virtual {v9, v8}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v9}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v8

    invoke-interface {v3, v4, v8}, Lyuc;->log(Ljava/lang/String;Ljava/lang/String;)V

    invoke-virtual {v7}, Ljava/util/ArrayList;->size()I

    move-result v8

    :goto_18
    if-ge v5, v8, :cond_25

    invoke-virtual {v7, v5}, Ljava/util/ArrayList;->get(I)Ljava/lang/Object;

    move-result-object v9

    add-int/lit8 v5, v5, 0x1

    check-cast v9, Lfg1;

    iget-object v10, v9, Lfg1;->a:Landroid/opengl/EGLSurface;

    iput-object v6, v9, Lfg1;->a:Landroid/opengl/EGLSurface;

    invoke-virtual {v1, v10}, Leg1;->d(Landroid/opengl/EGLSurface;)V

    invoke-virtual {v9, v1}, Lfg1;->c(Leg1;)V

    goto :goto_18

    :cond_25
    invoke-virtual {v7}, Ljava/util/ArrayList;->size()I

    move-result v1

    new-instance v5, Ljava/lang/StringBuilder;

    invoke-direct {v5}, Ljava/lang/StringBuilder;-><init>()V

    invoke-virtual {v5, v1}, Ljava/lang/StringBuilder;->append(I)Ljava/lang/StringBuilder;

    const-string v1, " drawers were released"

    invoke-virtual {v5, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v5}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v1

    invoke-interface {v3, v4, v1}, Lyuc;->log(Ljava/lang/String;Ljava/lang/String;)V

    invoke-virtual {v7}, Ljava/util/ArrayList;->clear()V

    iget-object v1, v2, Lgg1;->h:Lorg/webrtc/GlRectDrawer;

    invoke-virtual {v1}, Lorg/webrtc/GlRectDrawer;->release()V

    const-string v1, "Shared holder released"

    invoke-interface {v3, v4, v1}, Lyuc;->log(Ljava/lang/String;Ljava/lang/String;)V

    iget-object v1, v2, Lgg1;->g:Lorg/webrtc/VideoFrameDrawer;

    invoke-virtual {v1}, Lorg/webrtc/VideoFrameDrawer;->release()V

    const-string v1, "Frame drawer released"

    invoke-interface {v3, v4, v1}, Lyuc;->log(Ljava/lang/String;Ljava/lang/String;)V

    sget-object v1, Lzag;->a:Lzag;

    return-object v1

    :pswitch_c
    move-object/from16 v1, p1

    check-cast v1, Lwnf;

    iget-object v2, v0, Lst1;->receiver:Ljava/lang/Object;

    check-cast v2, Ltfd;

    invoke-virtual {v2, v1}, Ltfd;->f(Lwnf;)Lvnf;

    move-result-object v1

    return-object v1

    :pswitch_d
    move-object/from16 v1, p1

    check-cast v1, Lh0d;

    iget-object v2, v0, Lst1;->receiver:Ljava/lang/Object;

    check-cast v2, Ldz2;

    iget-object v2, v2, Ldz2;->a:Lone/me/chats/search/ChatsListSearchScreen;

    sget-object v3, Lone/me/chats/search/ChatsListSearchScreen;->I0:[Lwq7;

    invoke-static {v2}, Lbbi;->b(Lx14;)V

    invoke-virtual {v2}, Lone/me/chats/search/ChatsListSearchScreen;->D0()Lr03;

    move-result-object v2

    iget-object v3, v2, Lr03;->L0:Lde5;

    invoke-virtual {v1}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    iget-boolean v4, v1, Lh0d;->Z:Z

    if-eqz v4, :cond_26

    sget-object v5, Ls23;->c:Ls23;

    iget-wide v6, v1, Lh0d;->a:J

    sget-object v8, Lr3h;->Z:Lr3h;

    const/4 v9, 0x0

    const/4 v10, 0x4

    invoke-static/range {v5 .. v10}, Ls23;->V0(Ls23;JLr3h;Ljava/lang/String;I)Lhf4;

    move-result-object v1

    invoke-static {v3, v1}, Lsyg;->o(Lde5;Ljava/lang/Object;)V

    goto :goto_19

    :cond_26
    iget-wide v3, v1, Lh0d;->a:J

    invoke-virtual {v2, v3, v4}, Lr03;->x(J)V

    :goto_19
    sget-object v1, Lzag;->a:Lzag;

    return-object v1

    :pswitch_e
    move-object/from16 v1, p1

    check-cast v1, Ljava/lang/Number;

    invoke-virtual {v1}, Ljava/lang/Number;->longValue()J

    move-result-wide v3

    iget-object v1, v0, Lst1;->receiver:Ljava/lang/Object;

    check-cast v1, Lz7c;

    check-cast v1, Lone/me/profileedit/screens/memberpermissions/ProfileMemberPermissionsScreen;

    invoke-virtual {v1}, Lone/me/profileedit/screens/memberpermissions/ProfileMemberPermissionsScreen;->C0()Lo8c;

    move-result-object v1

    iget-object v5, v1, Lo8c;->z0:Lon7;

    invoke-interface {v5}, Lmm7;->isActive()Z

    move-result v5

    if-eqz v5, :cond_27

    goto :goto_1a

    :cond_27
    new-instance v5, Lg8c;

    invoke-direct {v5, v3, v4, v1, v6}, Lg8c;-><init>(JLo8c;Lkotlin/coroutines/Continuation;)V

    invoke-static {v1, v6, v5, v2}, Lsyg;->m(Lsyg;Lt44;Lei6;I)Lwwe;

    move-result-object v2

    iput-object v2, v1, Lo8c;->z0:Lon7;

    :goto_1a
    sget-object v1, Lzag;->a:Lzag;

    return-object v1

    :pswitch_f
    move-object/from16 v1, p1

    check-cast v1, Ljava/lang/Number;

    invoke-virtual {v1}, Ljava/lang/Number;->longValue()J

    move-result-wide v1

    iget-object v3, v0, Lst1;->receiver:Ljava/lang/Object;

    check-cast v3, Li2c;

    check-cast v3, Lone/me/profileedit/screens/adminpermissions/ProfileEditAdminPermissionsWidget;

    invoke-virtual {v3}, Lone/me/profileedit/screens/adminpermissions/ProfileEditAdminPermissionsWidget;->D0()Lx2c;

    move-result-object v3

    invoke-virtual {v3, v1, v2, v7}, Lx2c;->v(JZ)V

    sget-object v1, Lzag;->a:Lzag;

    return-object v1

    :pswitch_10
    move-object/from16 v1, p1

    check-cast v1, Lpzb;

    iget-object v2, v0, Lst1;->receiver:Ljava/lang/Object;

    check-cast v2, Lp0c;

    invoke-virtual {v2}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    sget-object v3, Lmzb;->a:Lmzb;

    invoke-static {v1, v3}, Lc0i;->a(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v3

    if-eqz v3, :cond_28

    sget-object v1, Lh0c;->a:Lh0c;

    goto :goto_1c

    :cond_28
    instance-of v3, v1, Lnzb;

    if-eqz v3, :cond_29

    check-cast v1, Lnzb;

    iget v3, v1, Lnzb;->a:I

    iput v3, v2, Lp0c;->Y:I

    new-instance v3, Ll0c;

    iget v1, v1, Lnzb;->a:I

    invoke-direct {v3, v1}, Ll0c;-><init>(I)V

    :goto_1b
    move-object v1, v3

    goto :goto_1c

    :cond_29
    instance-of v3, v1, Lozb;

    if-eqz v3, :cond_2a

    new-instance v3, Lk0c;

    check-cast v1, Lozb;

    iget-object v1, v1, Lozb;->a:Landroid/net/Uri;

    invoke-direct {v3, v1}, Lk0c;-><init>(Landroid/net/Uri;)V

    goto :goto_1b

    :goto_1c
    iget-object v2, v2, Lp0c;->Z:Lde5;

    invoke-static {v2, v1}, Lsyg;->o(Lde5;Ljava/lang/Object;)V

    sget-object v1, Lzag;->a:Lzag;

    return-object v1

    :cond_2a
    new-instance v1, Lkotlin/NoWhenBranchMatchedException;

    invoke-direct {v1}, Lkotlin/NoWhenBranchMatchedException;-><init>()V

    throw v1

    :pswitch_11
    move-object/from16 v1, p1

    check-cast v1, Landroid/view/View;

    iget-object v2, v0, Lst1;->receiver:Ljava/lang/Object;

    check-cast v2, Lone/me/profile/screens/avatars/ProfileAvatarsScreen;

    sget-object v3, Lone/me/profile/screens/avatars/ProfileAvatarsScreen;->y0:[Lwq7;

    invoke-virtual {v2}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    invoke-static {v7}, Lcvi;->a(I)Ls04;

    move-result-object v3

    invoke-virtual {v2}, Lone/me/profile/screens/avatars/ProfileAvatarsScreen;->R0()Lp0c;

    move-result-object v4

    invoke-virtual {v2}, Lone/me/profile/screens/avatars/ProfileAvatarsScreen;->S0()Landroidx/viewpager2/widget/ViewPager2;

    move-result-object v6

    invoke-virtual {v6}, Landroidx/viewpager2/widget/ViewPager2;->getCurrentItem()I

    move-result v6

    iget-object v8, v4, Lp0c;->b:Ltzb;

    iget v4, v4, Lp0c;->Y:I

    if-ne v6, v4, :cond_2b

    move v5, v7

    :cond_2b
    invoke-interface {v8, v5}, Ltzb;->d(Z)Ljava/util/List;

    move-result-object v4

    new-instance v5, Ljava/util/ArrayList;

    const/16 v6, 0xa

    invoke-static {v4, v6}, Lcb3;->l(Ljava/lang/Iterable;I)I

    move-result v6

    invoke-direct {v5, v6}, Ljava/util/ArrayList;-><init>(I)V

    invoke-interface {v4}, Ljava/lang/Iterable;->iterator()Ljava/util/Iterator;

    move-result-object v4

    :goto_1d
    invoke-interface {v4}, Ljava/util/Iterator;->hasNext()Z

    move-result v6

    if-eqz v6, :cond_2c

    invoke-interface {v4}, Ljava/util/Iterator;->next()Ljava/lang/Object;

    move-result-object v6

    check-cast v6, Llzb;

    new-instance v7, Lv04;

    invoke-virtual {v6}, Ljava/lang/Enum;->ordinal()I

    move-result v8

    iget-object v9, v6, Llzb;->a:Ljqf;

    const/4 v11, 0x0

    const/16 v12, 0x1c

    const/4 v10, 0x0

    invoke-direct/range {v7 .. v12}, Lv04;-><init>(ILoqf;Ljava/lang/Integer;Ljava/lang/Integer;I)V

    invoke-virtual {v5, v7}, Ljava/util/ArrayList;->add(Ljava/lang/Object;)Z

    goto :goto_1d

    :cond_2c
    invoke-interface {v3, v5}, Ls04;->i(Ljava/util/Collection;)Ls04;

    move-result-object v3

    invoke-interface {v3, v1}, Ls04;->t(Landroid/view/View;)Ls04;

    move-result-object v1

    invoke-interface {v1}, Ls04;->h()Ls04;

    move-result-object v1

    invoke-interface {v1}, Ls04;->c()Ls04;

    move-result-object v1

    invoke-interface {v1}, Ls04;->build()Lt04;

    move-result-object v1

    invoke-interface {v1, v2}, Lt04;->v(Lone/me/sdk/arch/Widget;)V

    sget-object v1, Lzag;->a:Lzag;

    return-object v1

    :pswitch_12
    move-object/from16 v1, p1

    check-cast v1, Ltr6;

    iget-object v2, v0, Lst1;->receiver:Ljava/lang/Object;

    check-cast v2, Lone/me/location/map/pick/PickLocationScreen;

    invoke-virtual {v2, v1}, Lone/me/location/map/pick/PickLocationScreen;->W(Ltr6;)V

    sget-object v1, Lzag;->a:Lzag;

    return-object v1

    :pswitch_13
    move-object/from16 v1, p1

    check-cast v1, Landroid/app/Activity;

    iget-object v2, v0, Lst1;->receiver:Ljava/lang/Object;

    check-cast v2, Ltma;

    invoke-virtual {v2}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    invoke-static {v1}, Ltma;->a(Landroid/app/Activity;)Z

    move-result v1

    invoke-static {v1}, Ljava/lang/Boolean;->valueOf(Z)Ljava/lang/Boolean;

    move-result-object v1

    return-object v1

    :pswitch_14
    move-object/from16 v1, p1

    check-cast v1, Lw5a;

    iget-object v2, v0, Lst1;->receiver:Ljava/lang/Object;

    check-cast v2, Ll7a;

    invoke-virtual {v2}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    if-eqz v1, :cond_2e

    iget v1, v1, Lw5a;->c:I

    iget v3, v2, Ll7a;->Z:I

    if-ne v1, v3, :cond_2d

    goto :goto_1e

    :cond_2d
    iput v1, v2, Ll7a;->Z:I

    iget-object v2, v2, Ll7a;->v0:Leie;

    new-instance v3, Lc6a;

    invoke-direct {v3, v1, v6}, Lc6a;-><init>(ILjava/lang/Integer;)V

    invoke-virtual {v2, v3}, Leie;->h(Ljava/lang/Object;)Z

    :cond_2e
    :goto_1e
    sget-object v1, Lzag;->a:Lzag;

    return-object v1

    :pswitch_15
    move-object/from16 v1, p1

    check-cast v1, Lw5a;

    iget-object v2, v0, Lst1;->receiver:Ljava/lang/Object;

    check-cast v2, Le6a;

    invoke-interface {v2, v1}, Le6a;->a(Lw5a;)V

    sget-object v1, Lzag;->a:Lzag;

    return-object v1

    :pswitch_16
    move-object/from16 v1, p1

    check-cast v1, Lai9;

    iget-object v2, v0, Lst1;->receiver:Ljava/lang/Object;

    check-cast v2, Lim9;

    iget-object v3, v2, Lim9;->o1:Ldsb;

    sget-object v5, Lim9;->P1:[Lwq7;

    aget-object v4, v5, v4

    iget-object v3, v3, Ldsb;->b:Ljava/lang/Object;

    check-cast v3, Lxl;

    invoke-static {v1}, Ljava/util/Collections;->singletonList(Ljava/lang/Object;)Ljava/util/List;

    move-result-object v4

    new-instance v5, Lh33;

    const/16 v6, 0x18

    invoke-direct {v5, v2, v6, v1}, Lh33;-><init>(Ljava/lang/Object;ILjava/lang/Object;)V

    invoke-virtual {v3, v4, v5}, Lxl;->c(Ljava/util/List;Loh6;)V

    sget-object v1, Lzag;->a:Lzag;

    return-object v1

    :pswitch_17
    move-object/from16 v1, p1

    check-cast v1, Ljava/lang/Number;

    invoke-virtual {v1}, Ljava/lang/Number;->longValue()J

    move-result-wide v1

    iget-object v3, v0, Lst1;->receiver:Ljava/lang/Object;

    check-cast v3, Lone/me/messages/list/ui/MessagesListWidget;

    sget-object v4, Lone/me/messages/list/ui/MessagesListWidget;->e1:[Lwq7;

    invoke-virtual {v3}, Lone/me/messages/list/ui/MessagesListWidget;->Q0()V

    invoke-virtual {v3}, Lone/me/messages/list/ui/MessagesListWidget;->P0()Lim9;

    move-result-object v3

    invoke-virtual {v3}, Lim9;->B()Lsx9;

    move-result-object v3

    invoke-virtual {v3, v1, v2}, Lsx9;->g(J)V

    sget-object v1, Lzag;->a:Lzag;

    return-object v1

    :pswitch_18
    move-object/from16 v1, p1

    check-cast v1, Lc99;

    iget-object v2, v0, Lst1;->receiver:Ljava/lang/Object;

    check-cast v2, Le99;

    invoke-virtual {v2}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    invoke-virtual {v1}, Ljava/lang/Enum;->ordinal()I

    move-result v1

    packed-switch v1, :pswitch_data_1

    new-instance v1, Lkotlin/NoWhenBranchMatchedException;

    invoke-direct {v1}, Lkotlin/NoWhenBranchMatchedException;-><init>()V

    throw v1

    :pswitch_19
    new-instance v2, Lv04;

    sget v3, Lzmc;->messages_list_context_action_share_post:I

    sget v1, Lkxa;->A:I

    new-instance v4, Ljqf;

    invoke-direct {v4, v1}, Ljqf;-><init>(I)V

    sget v1, Lwza;->v:I

    invoke-static {v1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v5

    sget v1, Luza;->T:I

    invoke-static {v1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v6

    const/4 v7, 0x4

    invoke-direct/range {v2 .. v7}, Lv04;-><init>(ILoqf;Ljava/lang/Integer;Ljava/lang/Integer;I)V

    goto/16 :goto_25

    :pswitch_1a
    new-instance v3, Lv04;

    sget v4, Lzmc;->messages_list_context_action_share_externally:I

    sget v1, Lurc;->chat_screen_action_share_externally:I

    new-instance v5, Ljqf;

    invoke-direct {v5, v1}, Ljqf;-><init>(I)V

    sget v1, Lwza;->z:I

    invoke-static {v1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v6

    sget v1, Luza;->T:I

    invoke-static {v1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v7

    const/4 v8, 0x4

    invoke-direct/range {v3 .. v8}, Lv04;-><init>(ILoqf;Ljava/lang/Integer;Ljava/lang/Integer;I)V

    :goto_1f
    move-object v2, v3

    goto/16 :goto_25

    :pswitch_1b
    new-instance v4, Lv04;

    sget v5, Lixa;->r:I

    sget v1, Lkxa;->n:I

    new-instance v6, Ljqf;

    invoke-direct {v6, v1}, Ljqf;-><init>(I)V

    sget v1, Liid;->t:I

    invoke-static {v1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v7

    sget v1, Luza;->T:I

    invoke-static {v1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v8

    const/4 v9, 0x4

    invoke-direct/range {v4 .. v9}, Lv04;-><init>(ILoqf;Ljava/lang/Integer;Ljava/lang/Integer;I)V

    :goto_20
    move-object v2, v4

    goto/16 :goto_25

    :pswitch_1c
    new-instance v5, Lv04;

    sget v6, Lixa;->A:I

    sget v1, Lkxa;->y:I

    new-instance v7, Ljqf;

    invoke-direct {v7, v1}, Ljqf;-><init>(I)V

    sget v1, Liid;->D:I

    invoke-static {v1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v8

    sget v1, Luza;->T:I

    invoke-static {v1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v9

    const/4 v10, 0x4

    invoke-direct/range {v5 .. v10}, Lv04;-><init>(ILoqf;Ljava/lang/Integer;Ljava/lang/Integer;I)V

    :goto_21
    move-object v2, v5

    goto/16 :goto_25

    :pswitch_1d
    new-instance v6, Lv04;

    sget v7, Lixa;->u:I

    sget v1, Lkxa;->s:I

    new-instance v8, Ljqf;

    invoke-direct {v8, v1}, Ljqf;-><init>(I)V

    sget v1, Liid;->F:I

    invoke-static {v1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v9

    sget v1, Luza;->T:I

    invoke-static {v1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v10

    const/4 v11, 0x4

    invoke-direct/range {v6 .. v11}, Lv04;-><init>(ILoqf;Ljava/lang/Integer;Ljava/lang/Integer;I)V

    :goto_22
    move-object v2, v6

    goto/16 :goto_25

    :pswitch_1e
    new-instance v7, Lv04;

    sget v8, Lixa;->B:I

    sget v1, Lkxa;->z:I

    new-instance v9, Ljqf;

    invoke-direct {v9, v1}, Ljqf;-><init>(I)V

    sget v1, Liid;->l:I

    invoke-static {v1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v10

    sget v1, Luza;->T:I

    invoke-static {v1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v11

    const/4 v12, 0x4

    invoke-direct/range {v7 .. v12}, Lv04;-><init>(ILoqf;Ljava/lang/Integer;Ljava/lang/Integer;I)V

    :goto_23
    move-object v2, v7

    goto/16 :goto_25

    :pswitch_1f
    new-instance v1, Lv04;

    sget v2, Lixa;->C:I

    sget v3, Lkxa;->B:I

    new-instance v4, Ljqf;

    invoke-direct {v4, v3}, Ljqf;-><init>(I)V

    sget v3, Liid;->y1:I

    invoke-static {v3}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v3

    sget v5, Luza;->T:I

    invoke-static {v5}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v5

    const/4 v6, 0x4

    move-object/from16 v20, v4

    move-object v4, v3

    move-object/from16 v3, v20

    invoke-direct/range {v1 .. v6}, Lv04;-><init>(ILoqf;Ljava/lang/Integer;Ljava/lang/Integer;I)V

    :goto_24
    move-object v2, v1

    goto/16 :goto_25

    :pswitch_20
    new-instance v2, Lv04;

    sget v3, Lixa;->x:I

    sget v1, Lkxa;->v:I

    new-instance v4, Ljqf;

    invoke-direct {v4, v1}, Ljqf;-><init>(I)V

    sget v1, Liid;->A1:I

    invoke-static {v1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v5

    sget v1, Luza;->T:I

    invoke-static {v1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v6

    const/4 v7, 0x4

    invoke-direct/range {v2 .. v7}, Lv04;-><init>(ILoqf;Ljava/lang/Integer;Ljava/lang/Integer;I)V

    goto/16 :goto_25

    :pswitch_21
    new-instance v3, Lv04;

    sget v4, Lixa;->t:I

    sget v1, Lkxa;->r:I

    new-instance v5, Ljqf;

    invoke-direct {v5, v1}, Ljqf;-><init>(I)V

    sget v1, Luza;->V:I

    invoke-static {v1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v6

    sget v1, Liid;->w:I

    invoke-static {v1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v7

    sget v1, Luza;->Q:I

    invoke-static {v1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v8

    invoke-direct/range {v3 .. v8}, Lv04;-><init>(ILoqf;Ljava/lang/Integer;Ljava/lang/Integer;Ljava/lang/Integer;)V

    goto/16 :goto_1f

    :pswitch_22
    new-instance v4, Lv04;

    sget v5, Lixa;->s:I

    sget v1, Lkxa;->q:I

    new-instance v6, Ljqf;

    invoke-direct {v6, v1}, Ljqf;-><init>(I)V

    sget v1, Luza;->V:I

    invoke-static {v1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v7

    sget v1, Liid;->w:I

    invoke-static {v1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v8

    sget v1, Luza;->Q:I

    invoke-static {v1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v9

    invoke-direct/range {v4 .. v9}, Lv04;-><init>(ILoqf;Ljava/lang/Integer;Ljava/lang/Integer;Ljava/lang/Integer;)V

    goto/16 :goto_20

    :pswitch_23
    new-instance v5, Lv04;

    sget v6, Lixa;->y:I

    sget v1, Lkxa;->w:I

    new-instance v7, Ljqf;

    invoke-direct {v7, v1}, Ljqf;-><init>(I)V

    sget v1, Liid;->L1:I

    invoke-static {v1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v8

    sget v1, Luza;->T:I

    invoke-static {v1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v9

    const/4 v10, 0x4

    invoke-direct/range {v5 .. v10}, Lv04;-><init>(ILoqf;Ljava/lang/Integer;Ljava/lang/Integer;I)V

    goto/16 :goto_21

    :pswitch_24
    new-instance v6, Lv04;

    sget v7, Lixa;->w:I

    sget v1, Lkxa;->u:I

    new-instance v8, Ljqf;

    invoke-direct {v8, v1}, Ljqf;-><init>(I)V

    sget v1, Liid;->j1:I

    invoke-static {v1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v9

    sget v1, Luza;->T:I

    invoke-static {v1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v10

    const/4 v11, 0x4

    invoke-direct/range {v6 .. v11}, Lv04;-><init>(ILoqf;Ljava/lang/Integer;Ljava/lang/Integer;I)V

    goto/16 :goto_22

    :pswitch_25
    new-instance v7, Lv04;

    sget v8, Lixa;->z:I

    sget v1, Lkxa;->x:I

    new-instance v9, Ljqf;

    invoke-direct {v9, v1}, Ljqf;-><init>(I)V

    sget v1, Liid;->M1:I

    invoke-static {v1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v10

    sget v1, Luza;->T:I

    invoke-static {v1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v11

    const/4 v12, 0x4

    invoke-direct/range {v7 .. v12}, Lv04;-><init>(ILoqf;Ljava/lang/Integer;Ljava/lang/Integer;I)V

    goto/16 :goto_23

    :pswitch_26
    new-instance v1, Lv04;

    sget v2, Lixa;->q:I

    sget v3, Lkxa;->m:I

    new-instance v4, Ljqf;

    invoke-direct {v4, v3}, Ljqf;-><init>(I)V

    sget v3, Liid;->t:I

    invoke-static {v3}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v3

    sget v5, Luza;->T:I

    invoke-static {v5}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v5

    const/4 v6, 0x4

    move-object/from16 v20, v4

    move-object v4, v3

    move-object/from16 v3, v20

    invoke-direct/range {v1 .. v6}, Lv04;-><init>(ILoqf;Ljava/lang/Integer;Ljava/lang/Integer;I)V

    goto/16 :goto_24

    :pswitch_27
    new-instance v2, Lv04;

    sget v3, Lixa;->v:I

    sget v1, Lkxa;->t:I

    new-instance v4, Ljqf;

    invoke-direct {v4, v1}, Ljqf;-><init>(I)V

    sget v1, Liid;->U1:I

    invoke-static {v1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v5

    sget v1, Luza;->T:I

    invoke-static {v1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v6

    const/4 v7, 0x4

    invoke-direct/range {v2 .. v7}, Lv04;-><init>(ILoqf;Ljava/lang/Integer;Ljava/lang/Integer;I)V

    :goto_25
    return-object v2

    :pswitch_28
    move-object/from16 v1, p1

    check-cast v1, Landroid/view/MotionEvent;

    iget-object v2, v0, Lst1;->receiver:Ljava/lang/Object;

    check-cast v2, Lone/me/sdk/messagewrite/MessageWriteWidget;

    sget-object v3, Lone/me/sdk/messagewrite/MessageWriteWidget;->K0:[Lwq7;

    invoke-virtual {v2}, Lx14;->getView()Landroid/view/View;

    move-result-object v3

    if-nez v3, :cond_2f

    goto :goto_26

    :cond_2f
    invoke-virtual {v2}, Lone/me/sdk/messagewrite/MessageWriteWidget;->G0()Lfc9;

    move-result-object v3

    invoke-virtual {v3}, Lfc9;->getText()Ljava/lang/CharSequence;

    move-result-object v3

    if-eqz v3, :cond_30

    invoke-static {v3}, Ls9f;->E(Ljava/lang/CharSequence;)Z

    move-result v3

    if-eqz v3, :cond_32

    :cond_30
    invoke-virtual {v2}, Lone/me/sdk/messagewrite/MessageWriteWidget;->G0()Lfc9;

    move-result-object v3

    invoke-virtual {v3}, Lfc9;->getSendActionState()Lzb9;

    move-result-object v3

    instance-of v3, v3, Lvb9;

    if-eqz v3, :cond_32

    invoke-virtual {v2}, Lone/me/sdk/messagewrite/MessageWriteWidget;->G0()Lfc9;

    move-result-object v3

    invoke-virtual {v3}, Lfc9;->getEmojiExpandableState()Ltb9;

    move-result-object v3

    sget-object v4, Ltb9;->a:Ltb9;

    if-eq v3, v4, :cond_31

    goto :goto_26

    :cond_31
    invoke-virtual {v2}, Lone/me/sdk/messagewrite/MessageWriteWidget;->M0()Lig9;

    move-result-object v2

    iget-object v2, v2, Lig9;->S0:Lsze;

    new-instance v3, Lof9;

    sget-object v4, Lr1d;->b:Lr1d;

    invoke-direct {v3, v4, v1}, Lof9;-><init>(Lr1d;Landroid/view/MotionEvent;)V

    invoke-virtual {v2, v6, v3}, Lsze;->m(Ljava/lang/Object;Ljava/lang/Object;)Z

    invoke-virtual {v2, v6}, Lsze;->setValue(Ljava/lang/Object;)V

    :cond_32
    :goto_26
    sget-object v1, Lzag;->a:Lzag;

    return-object v1

    :pswitch_data_0
    .packed-switch 0x0
        :pswitch_28
        :pswitch_18
        :pswitch_17
        :pswitch_16
        :pswitch_15
        :pswitch_14
        :pswitch_13
        :pswitch_12
        :pswitch_11
        :pswitch_10
        :pswitch_f
        :pswitch_e
        :pswitch_d
        :pswitch_c
        :pswitch_b
        :pswitch_a
        :pswitch_9
        :pswitch_8
        :pswitch_7
        :pswitch_6
        :pswitch_5
        :pswitch_4
        :pswitch_3
        :pswitch_2
        :pswitch_1
        :pswitch_0
    .end packed-switch

    :pswitch_data_1
    .packed-switch 0x0
        :pswitch_27
        :pswitch_26
        :pswitch_25
        :pswitch_24
        :pswitch_23
        :pswitch_22
        :pswitch_21
        :pswitch_20
        :pswitch_1f
        :pswitch_1e
        :pswitch_1d
        :pswitch_1c
        :pswitch_1b
        :pswitch_1a
        :pswitch_19
    .end packed-switch
.end method
